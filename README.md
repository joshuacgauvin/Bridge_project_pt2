...
The parametric models outperform the RSF at national scale. However, the RSF's hyperparameters were deliberately coarsened to fit ~1M bridges in memory (`min_samples_leaf=50`, 10% per-tree subsampling); a **cost-matched capacity check** (`ablation_state_dummies.ipynb`, Arm D: MA-specialist hyperparameters — 300 trees, leaf 5, full bootstrap — on 15,057-row national subsamples, 5 seeds, scored on the full test split) reaches pooled C = 0.7879 ± 0.0041 versus 0.7547 for the full coarsened forest, and a 50k-row run reaches 0.7973. So the coarsening does understate the RSF — a fine-grained forest on 2% of the data already beats the full coarse one, but most of the gap survives de-coarsening (the +0.129 gap to Cox narrows only to +0.096 at matched cost and +0.086 at 50k rows).
